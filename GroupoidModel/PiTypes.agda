...
module GroupoidModel.PiTypes where
...
module _ {l} where
  ...
  _◂ : ∀{Γ} {A : Ty {l} Γ} → Tm Γ A → GrpdFunctor Γ (Γ ,, A)
  _₀ (M ◂) γ = γ , (M ₀') γ
  _₁ (M ◂) p = p , (M ₁') p
  fid (_◂ {Γ} {A} M) x = ap (id (cat Γ) x ,_) goal'
    where
      open Category (cat Γ)
      D = λ z → Category.Morph (cat ((A ₀) x)) z ((M ₀') x)
      prf = cong (_$ ((M ₀') x)) (cong _₀ (fid A x))
      goal' = swap-pathover D prf _ _ (getPathOver (fid' M x))
  f∘ (_◂ {Γ} M) f g = Σ-≡ (refl , {!!}) -- ap ((f ∘ g) ,_) {!f∘' M g f!}
    ...
    record
      ...
      ; Morph = λ M N
        ...
        → isNatural _ _ (M ◂) (N ◂) (λ γ → id (cat Γ) _ ,
            subst (λ z → Morph (cat ((A ₀) γ)) z ((N ₀') γ))
                  (sym (cong (_$ _) $ cong _₀ $ fid A γ))
                  (τ γ))
  ...
  module _ {Γ} (A : Ty {l} Γ) (B : Ty {l} (Γ ,, A)) where
    ...
    f∘ (Π' γ) f g = Functor-≡' _ _ _ _ {!!} -- Functor-≡ {!!} {!!} {!!} {!!} {!!}
      ...
      _₁' app {γ , x} {γ' , x'} (p , q) = _∘_ (cat ((B ₀) (γ' , x'))) aux aux''''
        where
          ...
          aux'''' : Morph (cat $ (B ₀) (γ' , x'))
                          (p-q· ((M ₀' $ γ) ₀' $ x))
                          (id-q· ((M ₀' $ γ') ₀' $ p· x))
          aux'''' = substMorph {C = cat $ (B ₀) (γ' , x')} goal aux'''
            where
              fmpAB = fmp0 (Γ ,, A) B
              postulate goal : id-q· (p-id· ((((M ₀') $ γ) ₀') $ x)) ≡ p-q· ((((M ₀') $ γ) ₀') $ x)
              -- goal = begin
